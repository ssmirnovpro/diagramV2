...
@enduml`
    ...
    updateEditorStats() {
        ...
        if (!umlCode || !lineCountEl || !charCountEl) return;
...
@enduml`;
    ...
    openFullscreen() {
        ...
        if (!img || !modal || !fullscreenImg) return;
        ...
        try {
            ...
            if (apiResponse.ok) {
                ...
                if (apiDot) {
                    ...
                    if (apiStatusText) apiStatusText.textContent = 'API service is healthy';
                }
                ...
                if (krokiDot) {
                    if (statusData.kroki_service === 'healthy') {
                        ...
                        if (krokiStatusText) krokiStatusText.textContent = 'Kroki service is healthy';
                    } else {
                        krokiDot.className = 'status-dot unhealthy';
                        if (krokiStatusText) krokiStatusText.textContent = 'Kroki service is unavailable';
                    }
                }
            ...
            const apiStatusText = document.getElementById('apiStatusText');
            const krokiStatusText = document.getElementById('krokiStatusText');
            if (apiStatusText) apiStatusText.textContent = 'API service is unavailable';
            if (krokiStatusText) krokiStatusText.textContent = 'Kroki service is unavailable';
    ...
    updateZoomControls() {
        ...
        if (zoomInBtn) zoomInBtn.disabled = this.zoomLevel >= 3;
        if (zoomOutBtn) zoomOutBtn.disabled = this.zoomLevel <= 0.25;
        if (resetZoomBtn) resetZoomBtn.disabled = this.zoomLevel === 1;
    }
